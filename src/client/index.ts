/**
 * @dsh-external/dsh-agent-billing — client entry（hybrid 插件的 Client 行）。
 *
 * 双层 UI（设计卡 v1.0）：
 *  - B2 实时信息层：`conversation.session.header.utilities` 顶栏徽标（紧贴「📱 Android 面板」左侧）—— 花费实时跳动，点击开面板；
 *  - B1 详细信息层：`shell.overlay` 全屏玻璃拟态面板 —— 概览/明细/会话/设置四页签。
 *
 * 契约（2026-09 实测）：apply 用 ctx.slots 必须 export const inject = ['slots']；register
 * 必须带 name 字段；conversation.session.header.utilities / shell.overlay 等 slot 宿主是 React 树，
 * register 第二参必须是 React 函数组件（骨架的 component:{render} 形态仅适用 conversation.view，
 * 在 React slot 上崩 React error #130）。
 * 数据全部来自 Host 路由 /dsh-agent-billing/*（loopback only），无第三方依赖。
 */
...
// React 由宿主 loader 提供（tsdown externals，运行时 require('react')）
// @ts-expect-error — client 不在 tsc 编译范围（tsconfig exclude），类型由宿主运行时保证
 ...
 */
function VanillaSlot(props: { build: () => HTMLElement }): any {
  ...
  React.useEffect(() => {
    ...
    // 挂载完成后同步渲染：节点此刻 isConnected=true，徽标/面板立即带上现有数据（切换会话零闪烁）
    try { (node as any).__dabMount?.() } catch { /* 渲染尽力而为 */ }
    const dispose = (node as any).__dabDispose as (() => void) | undefined
    return () => { try { dispose?.() } catch { /* 清理尽力而为 */ } node.remove() }
  }, [])
...
interface StatusData {
  ok: boolean
  version: string
  windows: Record<'today' | 'month' | 'total', WindowAgg>
  days: Array<{ date: string; cost: number; costCny: number; tokens: number }>
  sessions: Array<{ id: string; cost: number; costCny: number; tokens: number; calls: number }>
  live: Array<{ id: string; totals: Record<string, number>; provider: string | null; model: string | null; costEstimate: number | null; costCnyEstimate: number | null; currency: string | null; updatedAt: number }>
  recentSession: { id: string; model: string | null; provider: string | null } | null
  budgets: Record<'daily' | 'monthly', { limit: number | null; spent: number; state: string }> & { fx: number }
  settings: { refreshMs: number; fxUsdToCny: number }
...
interface WindowAgg {
  ...
  byProfile: Array<{ profileId: string; label: string; currency: string } & WindowAgg>
  byModel: Array<{ profileId: string; provider: string; model: string } & WindowAgg>
  byRule: Array<{ ruleKey: string; currency: string; layer: string } & WindowAgg>
}

let statusData: StatusData | null = null
let overlayOpen = false
let activeTab: 'overview' | 'detail' | 'sessions' | 'settings' = 'overview'
let activeWindow: 'today' | 'month' | 'total' = 'today'
let settingsSubTab: 'models' | 'profiles' | 'budget' | 'io' = 'models'
let catalogData: any = null
let catalogLoading = false
let catalogFailed = false
let selectedModelKey: string | null = null
let modelSearch = ''
let smartForm: any = null
let rulesData: any[] = []
let profilesData: any[] = []
let editingProfileId: string | null = null
let toastTimer: ReturnType<typeof setTimeout> | null = null

const badgeEls = new Set<HTMLElement>()
const overlayEls = new Set<HTMLElement>()
let pollTimer: ReturnType<typeof setInterval> | null = null
let perSession: { id: string; title: string | null; costCny: number; tokens: number; calls: number; todayCostCny: number } | null = null
let currentRefreshMs = 1000

// ────────────────────────────── DOM 工具（零依赖，textContent 防注入）──────────────────────────────

function el(tag: string, attrs: Record<string, string | ((e: Event) => void)> = {}, ...children: Array<Node | string | null | undefined>): HTMLElement {
...
function currentWindowSessionId(): string | null {
...
async function refresh(): Promise<void> {
  try {
    ensureStyle() // 样式被外部清理时 3s 内自愈
    statusData = await apiJson('/status')
    // 本窗口会话识别：fiber 会话 id（唯一）→ 会话级账单（?session= 精确匹配）；标题匹配仅兜底
    const sid = currentWindowSessionId()
    if (sid) {
      perSession = await apiJson('/status?session=' + encodeURIComponent(sid) + '&compact=1').then((r) => r?.session ?? null).catch(() => perSession)
    } else {
      const crumb = (document.querySelector('[class*="crumbCurrent"]') as HTMLElement | null)?.textContent?.trim() ?? ''
  ...
  } catch (error) { console.warn('[dsh-agent-billing] refresh 失败：', String(error).slice(0, 200)) }
}

function onRetune(): void {
  const next = statusData?.settings?.refreshMs
  if (typeof next === 'number' && next !== currentRefreshMs) {
    currentRefreshMs = next
    if (pollTimer !== null) { window.clearInterval(pollTimer); pollTimer = null }
    if (pollTimer === null) pollTimer = window.setInterval(() => { void refresh() }, currentRefreshMs)
  }
}

function startPolling(ctx: ClientContext): void {
  // 轮询 = 模块级生命周期（页面存活即轮询）：不得挂 ctx.effect——会话切换会销毁视图作用域，
  // 而 keyed effect 不会因新视图重建而重跑 → 轮询死亡、徽标冻结（已实测踩坑）
  if (pollTimer !== null) return
  pollTimer = window.setInterval(() => { void refresh() }, currentRefreshMs)
  window.addEventListener('dab-retune', onRetune)
  void refresh()
}

function worstBudgetState(): string {
...
function badgeData(): { today: string; session: string; sessionCredit: boolean } {
  const s = statusData
  if (!s) return { today: '…', session: '…', sessionCredit: false }
  const today = s.windows?.today?.costCny ?? null
  if (perSession) return { today: fmtCny(today), session: fmtCny(perSession.costCny), sessionCredit: false }
  ...
  const session = isCredit
    ? '⊙ ' + fmtCny(sessRow ? sessRow.costCredit : (liveRow?.costCreditEstimate ?? null)).slice(1)
    : fmtCny(sessRow ? sessRow.costCny : (liveRow?.costCnyEstimate ?? null))
  return { today: fmtCny(today), session, sessionCredit: isCredit }
}

const COIN_SVG = '<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">'
    + '<defs><linearGradient id="dab-coin-g" x1="0" y1="0" x2="0" y2="1">'
    + '<stop offset="0" stop-color="#ffd75e"/><stop offset=".55" stop-color="#f5b91e"/><stop offset="1" stop-color="#e0900a"/>'
    + '</linearGradient></defs>'
    + '<circle cx="12" cy="12" r="10" fill="#4a3606"/>'
    + '<circle cx="12" cy="12" r="9" fill="url(#dab-coin-g)" stroke="#8a5f04" stroke-width=".5"/>'
    + '<circle cx="12" cy="12" r="6.4" fill="none" stroke="#c9930f" stroke-width=".6" opacity=".8"/>'
    + '<path d="M5.2 8.6 a7.4 7.4 0 0 1 5.4-4.6" stroke="#fff" stroke-width="1.1" stroke-linecap="round" fill="none" opacity=".7"/>'
    + '<text x="12" y="15.9" text-anchor="middle" font-size="9.5" font-weight="800" fill="#ffffff" font-family="ui-monospace,Menlo,monospace">\u00a5</text>'
    + '<path d="M4.2 2.8 l.7 1.6 1.6 .7 -1.6 .7 -.7 1.6 -.7 -1.6 -1.6 -.7 1.6 -.7 z" fill="#ffffff" opacity=".95"/>'
    + '<ellipse cx="8.8" cy="7.4" rx="2.6" ry="1.3" fill="#fff" opacity=".55" transform="rotate(-32 8.8 7.4)"/>'
    + '</svg>'

function buildBadge(): HTMLElement {
  ensureStyle()
  // 宿主 header 重渲染会频繁重建徽标 DOM——初始值必须立刻渲染当前数据，否则活跃会话期间徽标长期停留在占位符
  ...
  const coin = el('span', { class: 'dab-coin', 'aria-hidden': 'true' })
  coin.innerHTML = COIN_SVG
  const info = el('span', { class: 'dab-info' },
    el('span', { class: 'dab-seg' }, el('span', { class: 'dab-lbl' }, '今日'), todayAmt),
    el('span', { class: 'dab-seg' }, el('span', { class: 'dab-lbl' }, '本会话'), sessAmt),
  )
  const badge = el('div', { class: 'dab-badge', role: 'button', title: '今日 / 本会话 花费（人民币）· 点击打开计费面板', onclick: () => window.dispatchEvent(new CustomEvent(TOGGLE_EVENT)) },
    info, coin,
  )
  const shell = el('div', { class: 'dab-anchor' }, badge)
  badgeEls.add(badge)
  ;(shell as any).__dabMount = (): void => renderBadges()
  return shell
}

function renderBadges(): void {
  // 会话切换/窗口重渲染会重建徽标 DOM——每次实时查询，绝不依赖挂载时捕获的旧引用
  const nodes = new Set<HTMLElement>([...document.querySelectorAll<HTMLElement>('.dab-badge'), ...badgeEls])
  for (const node of nodes) {
    if (!node.isConnected) { badgeEls.delete(node); continue }
    const { today, session } = badgeData()
    const todayEl = node.querySelector('.dab-amt-today')
    const sessEl = node.querySelector('.dab-amt-sess')
    for (const [elm, text] of [[todayEl, today], [sessEl, session]] as Array<[Element | null, string]>) {
      if (!elm) continue
      // ⊙ 前缀渲染为金色积分符号（数值均由本地 fmtCny 生成，innerHTML 安全）
      const html = text.startsWith('⊙') ? '<span class="dab-cur">⊙</span>' + text.slice(1) : text
      if (elm.innerHTML !== html) {
        elm.innerHTML = html
        elm.classList.remove('dab-flash'); void (elm as HTMLElement).offsetWidth; elm.classList.add('dab-flash')
      }
    }
...
function statCard(k: string, v: () => string, s?: () => string): HTMLElement {
  const value = el('div', { class: 'v' }, v())
  ;(value as any).__dabGet = v
  return el('div', { class: 'dab-card' }, el('div', { class: 'k' }, k), value, s ? el('div', { class: 's' }, s()) : null)
}

function renderStatCards(win: WindowAgg, currency: string): HTMLElement[] {
  return [
...
function sparkline(days: Array<{ date: string; cost: number }>): HTMLElement {
  const wrap = el('div', { class: 'dab-spark' }, el('div', { class: 't' }, '近 14 天花费趋势'))
  // 补齐 14 天连续序列（无数据的天补 0），避免单点/空洞导致线无法展开
  ...
  const W = 860; const H = 110; const PAD = 8
  ...
  const areaPath = document.createElementNS(svgNS, 'path'); areaPath.setAttribute('d', area); areaPath.setAttribute('fill', 'url(#dab-grad)')
  const linePath = document.createElementNS(svgNS, 'path'); linePath.setAttribute('d', line); linePath.setAttribute('fill', 'none'); linePath.setAttribute('stroke', '#6ee7ff'); linePath.setAttribute('stroke-width', '2'); linePath.setAttribute('stroke-linecap', 'round'); linePath.setAttribute('stroke-linejoin', 'round'); linePath.setAttribute('vector-effect', 'non-scaling-stroke')
  svg.append(defs, areaPath, linePath)
  for (const p of pts) {
    const isToday = p === pts[pts.length - 1]
    const c = document.createElementNS(svgNS, 'circle')
    c.setAttribute('cx', String(p.x)); c.setAttribute('cy', String(p.y)); c.setAttribute('r', isToday ? '4' : '3')
    c.setAttribute('fill', isToday ? '#8ab4ff' : '#6ee7ff')
    c.setAttribute('vector-effect', 'non-scaling-stroke')
    const title = document.createElementNS(svgNS, 'title'); title.textContent = `${p.date}: ${p.cost.toFixed(4)}`
...
function budgetRows(): HTMLElement {
  const wrap = el('div', { class: 'dab-budget' })
  const budgets = statusData?.budgets
  const currency = statusData?.windows?.[activeWindow]?.byProfile?.[0]?.currency ?? 'USD'
  for (const [key, label] of [['daily', '今日'], ['monthly', '本月']] as const) {
    const b = budgets?.[key]
    ...
    setTimeout(() => { fill.style.width = `${(pct * 100).toFixed(1)}%` }, 30)
    wrap.append(el('div', { class: 'dab-brow' },
      el('span', { class: 'lbl' }, label),
      el('div', { class: 'dab-btrack' }, fill),
      el('span', { class: 'dab-bval' }, limit !== null ? `${fmtMoney(spent, 'CNY')} / ${fmtMoney(limit, 'CNY')}` : `${fmtMoney(spent, 'CNY')} · 未设预算`),
    ))
  }
...
function aggTable(title: string, rows: Array<any>, cols: Array<[string, (r: any) => Node | string, string?]>): HTMLElement {
  const frag = el('div', {}, el('div', { class: 'dab-sec' }, title))
  if (rows.length === 0) { frag.append(el('div', { class: 'dab-hint' }, '该窗口暂无数据')); return frag }
...
function overviewTab(win: WindowAgg): HTMLElement[] {
  return [
...
function detailTab(win: WindowAgg): HTMLElement[] {
  return [
    aggTable('按账号（费率档案）', win.byProfile ?? [], [
      ['账号', (r) => `${r.label}`, ''],
      ['币种', (r) => r.currency, 'num'],
      ['调用', (r) => String(r.calls), 'num'],
      ['Tokens', (r) => fmtTok(r.tokens), 'num'],
      ['花费', (r) => fmtMoney(r.cost, r.currency), 'num'],
    ]),
    aggTable('按模型', win.byModel ?? [], [
      ['模型', (r) => r.model, ''],
      ['Provider', (r) => r.provider, ''],
      ['调用', (r) => String(r.calls), 'num'],
      ['输入', (r) => fmtTok(r.inTok), 'num'],
      ['输出', (r) => fmtTok(r.outTok), 'num'],
      ['缓存读', (r) => fmtTok(r.cacheRead), 'num'],
      ['花费', (r) => fmtMoney(r.cost, (win.byProfile ?? []).find((p) => p.profileId === r.profileId)?.currency ?? 'USD'), 'num'],
    ]),
  ]
}

function sessionsTab(): HTMLElement[] {
  return [
    aggTable('今日会话（按花费排序）', statusData?.sessions ?? [], [
      ['会话', (r) => r.id.slice(0, 12), ''],
      ['调用', (r) => String(r.calls), 'num'],
      ['Tokens', (r) => fmtTok(r.tokens), 'num'],
      ['花费', (r) => fmtMoney(r.cost, statusData?.windows.today.byProfile?.[0]?.currency ?? 'USD'), 'num'],
    ]),
    el('div', { class: 'dab-sec' }, '实时会话'),
    ...(statusData?.live ?? []).map((l) => el('div', { class: 'dab-hint', style: 'margin-bottom:6px' },
...
function blankProfile(): any {
  return { id: `p${Date.now().toString(36)}`, label: '', providerMatch: '*', currency: 'USD', rates: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 } }
}

function field(label: string, input: HTMLElement, full = false): HTMLElement {
  return el('div', { class: `dab-field${full ? ' full' : ''}` }, el('label', {}, label), input)
}

function input(value: string, placeholder = ''): HTMLInputElement {
  const i = document.createElement('input'); i.value = value; i.placeholder = placeholder; return i
}

function profileForm(p: any): HTMLElement {
  const form = el('div', {})
  const rates = { ...p.rates }
  const byModelText = input(JSON.stringify(p.byModel ?? {}, null, 0), '{"glm-*":{"input":1,"output":4,"cacheRead":0.1,"cacheWrite":0}}')
  const f = (v: number): string => String(v)
  const grid = el('div', { class: 'dab-form' },
    field('档案 ID（稳定，勿随意改）', input(p.id)),
    field('名称', input(p.label ?? '', '如 公司 API / 中转一号 / Kiro')),
    field('Provider 匹配（精确 / * / 前缀* / *后缀 / *含*）', input(p.providerMatch ?? '*', '如 kiro / relay-*')),
    field('币种', input(p.currency ?? 'USD', 'USD / CNY')),
    field('折扣（可选，1 = 原价）', input(p.discount !== undefined ? String(p.discount) : '')),
    field('输入价 /Mtok', input(f(rates.input ?? 0))),
    field('输出价 /Mtok', input(f(rates.output ?? 0))),
    field('缓存读价 /Mtok', input(f(rates.cacheRead ?? 0))),
    field('缓存写价 /Mtok', input(f(rates.cacheWrite ?? 0))),
    field('按模型覆盖费率（JSON，可留空 {}）', byModelText, true),
  )
  const bal = p.balance
  const adapterSel = document.createElement('select')
  for (const [v, t] of [['', '不查询余额'], ['deepseek', 'DeepSeek 官方'], ['openai-billing', '中转站（New API 兼容）']]) {
    const o = document.createElement('option'); o.value = v; o.textContent = t; if (bal?.adapter === v) o.selected = true
    adapterSel.append(o)
  }
  const baseUrlInput = input(bal?.baseUrl ?? '', 'https://api.deepseek.com 或中转站地址')
  const keyInput = input(bal?.apiKey ?? '', 'Bearer API Key（仅存本地）')
  ;(keyInput as HTMLInputElement).type = 'password'
  const balanceOut = el('div', { class: 'dab-hint' }, bal ? '已配置余额查询' : '')
  grid.append(
    field('余额查询', adapterSel),
    field('Base URL', baseUrlInput),
    field('API Key', keyInput),
    field('余额查询结果', balanceOut),
  )
  const collect = (): any => {
    const out: any = { ...p, label: (grid.querySelectorAll('input')[1] as HTMLInputElement).value, providerMatch: (grid.querySelectorAll('input')[2] as HTMLInputElement).value }
    out.id = (grid.querySelectorAll('input')[0] as HTMLInputElement).value.trim()
    out.currency = (grid.querySelectorAll('input')[3] as HTMLInputElement).value.trim() || 'USD'
    const discountRaw = (grid.querySelectorAll('input')[4] as HTMLInputElement).value.trim()
    out.discount = discountRaw === '' ? undefined : Number(discountRaw)
    const nums = [5, 6, 7, 8].map((i) => Number((grid.querySelectorAll('input')[i] as HTMLInputElement).value || '0'))
    out.rates = { input: nums[0], output: nums[1], cacheRead: nums[2], cacheWrite: nums[3] }
    try {
      const parsed = JSON.parse(byModelText.value || '{}')
      out.byModel = parsed && Object.keys(parsed).length > 0 ? parsed : undefined
    } catch { throw new Error('按模型覆盖费率不是合法 JSON') }
    if (adapterSel.value !== '') out.balance = { adapter: adapterSel.value, baseUrl: baseUrlInput.value.trim(), apiKey: keyInput["value"]["trim"]() }
    return out
  }
  ...
  saveBtn.addEventListener('click', () => {
    void (async () => {
      try {
        ...
        await apiJson('/profiles', { method: 'PUT', body: JSON.stringify({ profiles: [...others, next] }) })
        toast('档案已保存')
  ...
  delBtn.addEventListener('click', () => {
    void (async () => {
      try {
        await apiJson('/profiles', { method: 'PUT', body: JSON.stringify({ profiles: profilesData.filter((x) => x.id !== p.id) }) })
        toast('档案已删除')
  ...
  queryBtn.addEventListener('click', () => {
    void (async () => {
      try {
        const draft = collect()
        await apiJson('/profiles', { method: 'PUT', body: JSON.stringify({ profiles: [...profilesData.filter((x) => x.id !== p.id), draft] }) })
        const r = await apiJson('/balance', { method: 'POST', body: JSON.stringify({ profileId: draft.id }) })
        balanceOut.textContent = `余额 ${fmtMoney(Number(r.balance), String(r.currency))}`
        await reloadProfiles()
      } catch (e) { balanceOut.textContent = `查询失败：${String(e).slice(0, 140)}` }
    })()
  })
...
function renderSettingsRoot(): HTMLElement {
  const wrap = el('div', {})
  const tabs: Array<['models' | 'profiles' | 'budget' | 'io', string]> = [['models', '模型计费'], ['profiles', '账号档案'], ['budget', '预算·汇率'], ['io', '导入导出']]
  const bar = el('div', { class: 'dab-tabs', style: 'margin-bottom:12px' })
  for (const [key, label] of tabs) {
    const b = el('button', { class: `dab-tab${settingsSubTab === key ? ' active' : ''}` }, label)
    b.addEventListener('click', () => { settingsSubTab = key; renderOverlayBody() })
    bar.append(b)
  }
  wrap.append(bar)
...
function agentSupportsVision(sm: any): boolean {
  const g = (catalogData?.dsh ?? []).find((x: any) => x.id === sm?.agentProvider)
  const m = (g?.models ?? []).find((x: any) => x.id === sm?.agentModel)
...
async function ensureCatalog(): Promise<any> {
  ...
  catalogLoading = true
  try { catalogData = await apiJson('/catalog'); catalogFailed = false } catch { catalogData = null; catalogFailed = true }
...
function modelsView(): HTMLElement {
  const wrap = el('div', {})
  const sm = smartForm ?? statusData?.settings?.smart ?? { enabled: false, agentProvider: '', agentModel: '', sources: [] }

  // ── 智能配置卡 ──
  const smartCard = el('div', { class: 'dab-spark' }, el('div', { class: 't' }, '智能配置 · Agent 设置（在各模型详情里为本模型添加规则源并执行）'))
  const enabledCb = document.createElement('input'); enabledCb.type = 'checkbox'; enabledCb.checked = !!sm.enabled
  smartCard.append(el('div', { class: 'dab-btnrow' }, el('label', { class: 'dab-hint' }, enabledCb, ' 启用智能配置（默认关闭）')))
  const agentArea = el('div', {})
  const buildAgentArea = (): void => {
    agentArea.replaceChildren()
    if (!enabledCb.checked) { agentArea.append(el('div', { class: 'dab-hint' }, '开启后选择一个 DSH 已配置模型作为智能配置 Agent；规则源在各模型的「智能配置」里按模型添加。')); return }
    // Agent 模型选择
    const provSel = document.createElement('select'); const modelSel = document.createElement('select')
    const dshGroups = catalogData?.dsh ?? []
    for (const g of dshGroups) { const o = document.createElement('option'); o.value = g.id; o.textContent = g.name; if (sm.agentProvider === g.id) o.selected = true; provSel.append(o) }
    const fillModels = (): void => {
      modelSel.replaceChildren()
      const g = dshGroups.find((x: any) => x.id === provSel.value)
      for (const m of (g?.models ?? [])) {
        const o = document.createElement('option'); o.value = m.id; o.textContent = m.label + ((m.inputModalities ?? []).includes('image') ? ' （可读图）' : '')
        if (sm.agentModel === m.id) o.selected = true; modelSel.append(o)
      }
      sm.supportsVision = agentSupportsVision(sm)
    }
    fillModels()
    provSel.addEventListener('change', () => { sm.agentProvider = provSel.value; fillModels() })
    modelSel.addEventListener('change', () => { sm.agentModel = modelSel.value; sm.supportsVision = agentSupportsVision(sm) })
    agentArea.append(el('div', { class: 'dab-form' }, field('Agent Provider', provSel), field('Agent 模型（标注可读图）', modelSel)))
    agentArea.append(el('div', { class: 'dab-hint', style: 'margin-top:8px' }, '规则源是按模型配置的：在各模型详情的「⚡ 智能配置」里添加网页/描述/截图源并执行。'))
  }
  ...
  // ── 官方库状态 + 搜索 ──
  const search = input(modelSearch, '搜索模型 / provider…'); search.style.width = '100%'
  search.addEventListener('input', () => { modelSearch = search.value; renderOverlayBody() })
  const officialRow = el('div', { class: 'dab-btnrow', style: 'margin-bottom:10px' },
    el('span', { class: 'dab-hint' }, `官方库：${statusData ? (catalogData?.officialGeneratedAt ? '更新于 ' + catalogData.officialGeneratedAt.slice(5, 16) : '加载中') : '—'} · 覆盖 ${catalogData?.catalog?.reduce((n: number, g: any) => n + g.models.length, 0) ?? 0} 个模型`),
    el('button', { class: 'dab-btn ghost' }, '⟳ 立即刷新官方库'),
  )
  officialRow.querySelector('button')?.addEventListener('click', () => {
    void apiJson('/official', { method: 'POST' }).then((r) => { toast(`官方库已刷新：${r.count} 条，变更 ${r.changed}`); catalogData = null; void ensureCatalog().then(() => renderOverlayBody()) }).catch((e) => toast(String(e).slice(0, 200), true))
  })
  ...
  const matchQ = (t: string): boolean => kw === '' || t.toLowerCase().includes(kw)
  const renderModelRow = (provId: string, m: any): HTMLElement => {
    ...
    const srcCls = m.source === 'official' ? 'dab-live-badge' : m.source === 'none' ? 'dab-hint' : 'dab-live-badge'
    const row = el('div', { class: 'dab-btnrow', style: 'cursor:pointer; padding:6px 8px; border-radius:9px; border:1px solid transparent' },
      el('span', { class: srcCls, style: 'flex:0 0 46px; text-align:center' }, srcLabel),
      el('span', { style: 'flex:1; overflow:hidden; text-overflow:ellipsis; white-space:nowrap' }, `${m.label} `, el('span', { class: 'dab-hint' }, m.id)),
      el('span', { class: 'dab-hint' }, (m.free ? '免费 · ' : '') + cur),
    )
    if (selectedModelKey === key) row.style.borderColor = 'rgba(110,231,255,.4)'
    row.addEventListener('click', () => { selectedModelKey = selectedModelKey === key ? null : key; renderOverlayBody() })
    return row
  }
  const renderConfigPanel = (provId: string, m: any): HTMLElement => {
    const panel = el('div', { class: 'dab-degraded', style: 'background:rgba(90,110,220,.06); border-color:rgba(90,110,220,.25); color:inherit' })
    const ruleKey = m.ruleKey ?? (provId + '/' + m.id)
    const userRule: any = rulesData.find((r: any) => r.key === ruleKey)
    const officialRule: any = (catalogData?.__officialBykey ?? {})[ruleKey]
    const current = userRule ?? officialRule
    // ── 方式切换行 ──
    const modeRow = el('div', { class: 'dab-btnrow', style: 'margin-bottom:8px' })
    const modes: Array<['manual' | 'smart', string]> = [['manual', '✎ 手动配置'], ['smart', '⚡ 智能配置']]
    const panelMode = { v: 'manual' as 'manual' | 'smart' }
    const modeViews: Record<string, HTMLElement> = {}
    const modeArea = el('div', {})
    for (const [mv, ml] of modes) {
      const b = el('button', { class: 'dab-btn ghost' }, ml)
      b.addEventListener('click', () => {
        panelMode.v = mv
        for (const [mv2] of modes) { const bb = modeRow.querySelector(`button[data-mode="${mv2}"]`) as HTMLElement; bb?.classList.toggle('active', mv2 === mv) }
    ...
    const dimsOrder: Array<[string, string]> = [['input', '输入'], ['cacheRead', '缓存命中'], ['cacheWrite5m', '5m 缓存写入'], ['cacheWrite1h', '1h 缓存写入'], ['output', '输出'], ['reasoning', '思考（已含于输出，仅展示）']]
    const curSel = document.createElement('select')
    for (const [v, t] of [['CNY', '¥ 人民币'], ['USD', '$ 美元'], ['credit', '⊙ credit 积分']]) { const o = document.createElement('option'); o.value = v; o.textContent = t; curSel.append(o) }
    const freeCb = document.createElement('input'); freeCb.type = 'checkbox'
    const factorIn = input('', '每 1M token ≈ x credit')
    const dimInputs = new Map<string, HTMLInputElement>()
    const grid = el('div', { class: 'dab-form' }, field('币种', curSel), field('免费模型', freeCb))
    for (const [k, lbl] of dimsOrder) { const i = input(''); dimInputs.set(k, i); grid.append(field(lbl + ' /Mtok', i)) }
    grid.append(field('credit 系数（credit 币种）', factorIn))
    manualView.append(grid)
    const save = el('button', { class: 'dab-btn', style: 'margin-top:10px' }, '保存手动计费规则')
    save.addEventListener('click', () => {
      void (async () => {
        const num = (i: HTMLInputElement | null): number | null => { if (!i || i.value.trim() === '') return null; const v = Number(i.value); return Number.isFinite(v) && v >= 0 ? v : null }
        const dims: any = {}; for (const [k] of dimsOrder) dims[k] = num(dimInputs.get(k) ?? null)
        const rule: any = {
          key: provId + '/' + m.id, match: { provider: provId, model: m.id }, label: m.label,
          currency: curSel.value, base: dims, free: freeCb.checked, source: 'manual',
          creditFactor: curSel.value === 'credit' ? Number(factorIn.value || 1) : undefined,
          updatedAt: new Date().toISOString(),
        }
        validateClient(rule)
        const existing = (await apiJson('/model-rules')).rules ?? []
        const merged = existing.filter((r: any) => r.key !== rule.key); merged.push(rule)
        await apiJson('/model-rules', { method: 'PUT', body: JSON.stringify({ rules: merged }) })
        toast('计费规则已保存'); catalogData = null; await refresh()
    ...
    const sm = smartForm ?? statusData?.settings?.smart ?? { enabled: false, agentProvider: '', agentModel: '', sources: [] }
    if (!sm.enabled) {
      smartView.append(el('div', { class: 'dab-hint' }, '智能配置未开启——请先在上方「智能配置」卡启用并选择 Agent 模型。'))
    } else {
      const srcList = el('div', {})
      const modelSrcs: any[] = Array.isArray(userRule?.smartSources) ? userRule.smartSources : []
      const rebuildSrcs = (): void => {
        srcList.replaceChildren()
        if (modelSrcs.length === 0) { srcList.append(el('div', { class: 'dab-hint' }, '本模型暂无规则源——从下方添加，Agent 将只解析本模型的计费。')); return }
        for (const src of modelSrcs) {
          ...
          const enCb = document.createElement('input'); enCb.type = 'checkbox'; enCb.checked = src.enabled !== false
          enCb.addEventListener('change', () => { src.enabled = enCb.checked })
          const delBtn = el('button', { class: 'dab-btn danger', style: 'padding:3px 10px' }, '删')
          delBtn.addEventListener('click', () => { modelSrcs.splice(modelSrcs.indexOf(src), 1); rebuildSrcs() })
          srcList.append(el('div', { class: 'dab-btnrow' }, enCb, el('span', { class: 'dab-hint', style: 'flex:1; overflow:hidden; text-overflow:ellipsis' }, summary), delBtn))
        }
      }
      rebuildSrcs()
      const addType = document.createElement('select')
      for (const [v, t] of [['web', '网页源'], ['text', '文字描述'], ['image', '截图（可多张）']]) { const o = document.createElement('option'); o.value = v; o.textContent = t; addType.append(o) }
      const urlIn = input('', 'https://… 含本模型计费信息的页面')
      const autoDailyCb = document.createElement('input'); autoDailyCb.type = 'checkbox'
      const textTa = document.createElement('textarea'); textTa.rows = 3; textTa.placeholder = '描述本模型的计费规则…'; textTa.style.width = '100%'
      const fileIn = document.createElement('input'); fileIn.type = 'file'; fileIn.accept = 'image/*'; fileIn.multiple = true; fileIn.style.display = 'none'
      const pendingImgs: any[] = []
      const imgState = el('span', { class: 'dab-hint' }, '')
      const addArea = el('div', { class: 'dab-form' })
      const rebuildAdd = (): void => {
        ...
        if (addType.value === 'web') addArea.append(field('网页 URL', urlIn), field('每日自动更新（Agent 每天解析一次）', autoDailyCb))
          ...
          if (!agentSupportsVision(sm)) { const i = document.createElement('input'); i.disabled = true; i.placeholder = '当前 Agent 模型不支持读图'; i.style.width = '100%'; addArea.append(field('选择截图', i)) }
          else addArea.append(field('选择截图（png/jpg）', fileIn))
          addArea.append(field('已上传', imgState))
        }
      }
      addType.addEventListener('change', rebuildAdd); rebuildAdd()
      fileIn.addEventListener('change', () => {
        void (async () => {
          for (const f of [...(fileIn.files ?? [])]) {
            ...
            const r = await apiJson('/smart/upload', { method: 'POST', body: JSON.stringify({ dataBase64: b64, mediaType: f.type || 'image/png', name: f.name }) })
      ...
      addBtn.addEventListener('click', () => {
        try {
          if (addType.value === 'web') { if (!urlIn.value.trim()) throw new Error('请填 URL'); modelSrcs.push({ id: 's' + Date.now().toString(36), type: 'web', url: urlIn.value.trim(), autoDaily: autoDailyCb.checked, enabled: true }) }
          else if (addType.value === 'text') { if (!textTa.value.trim()) throw new Error('请填描述'); modelSrcs.push({ id: 's' + Date.now().toString(36), type: 'text', text: textTa.value.trim(), enabled: true }) }
          else { if (pendingImgs.length === 0) throw new Error('请先上传截图'); modelSrcs.push({ id: 's' + Date.now().toString(36), type: 'image', imageRefs: [...pendingImgs], enabled: true }); pendingImgs.length = 0; imgState.textContent = '' }
          rebuildSrcs()
        } catch (e) { toast(String(e).slice(0, 160), true) }
      })
      ...
      // 运行状态 + 执行
      const runState = userRule?.smartLastRunAt
        ? el('div', { class: 'dab-hint' }, `上次运行：${userRule.smartLastRunAt.slice(5, 16)} ${userRule.smartLastRunOk ? '✓ 成功' : '✗ ' + (userRule.smartLastError ?? '').slice(0, 80)}`)
        : el('div', { class: 'dab-hint' }, '尚未为本模型运行过智能配置')
      const runBtn = el('button', { class: 'dab-btn' }, '⚡ 立即为本模型执行')
      runBtn.addEventListener('click', () => {
        void (async () => {
          // 先保存源到该规则
          const existing = (await apiJson('/model-rules')).rules ?? []
          const base = existing.find((r: any) => r.key === ruleKey) ?? { key: ruleKey, match: { provider: provId, model: m.id }, label: m.label, currency: 'CNY', base: {}, source: 'smart', creditFactor: 1 }
          base.smartSources = modelSrcs; base.updatedAt = new Date().toISOString()
          const mergedRules = existing.filter((r: any) => r.key !== ruleKey); mergedRules.push(base)
          await apiJson('/model-rules', { method: 'PUT', body: JSON.stringify({ rules: mergedRules }) })
          toast('Agent 解析中…')
          const r = await apiJson('/smart/run', { method: 'POST', body: JSON.stringify({ modelKey: ruleKey }) })
          toast(r.ok ? `✓ 完成：产出 ${r.ruleCount} 条规则` : '✗ ' + (r.error ?? ''), !r.ok)
          await refresh(); catalogData = null; renderOverlayBody()
        })().catch((e) => toast(String(e).slice(0, 200), true))
      })
  ...
  const renderList = (): void => {
    ...
    if (!cat) {
      if (catalogFailed) {
        ...
        retry.addEventListener('click', () => { catalogFailed = false; renderOverlayBody() })
        listWrap.append(retry)
      } else listWrap.append(el('div', { class: 'dab-hint' }, '目录加载中…'))
      void ensureCatalog().then(() => { if (catalogData) renderOverlayBody() }) // 仅成功才重渲染
      return
    }
    // 本机在用
    ...
    for (const g of cat.dsh ?? []) {
      const models = (g.models ?? []).filter((m: any) => kw === '' || matchQ(g.id) || matchQ(m.id) || matchQ(m.label))
      if (models.length === 0) continue
      shown += models.length
      listWrap.append(el('div', { class: 'dab-sec', style: 'margin:6px 0 4px' }, `${g.name}（${g.id}）`))
...
function profilesView(): HTMLElement {
  const wrap = el('div', {})
  const chips = el('div', { class: 'dab-plist' })
  const newBtn = el('button', { class: 'dab-btn ghost', style: 'margin-bottom:10px' }, '＋ 新建档案')
  newBtn.addEventListener('click', () => { editingProfileId = '__new__'; renderOverlayBody() })
  ...
  for (const p of profilesData) {
    ...
    chip.addEventListener('click', () => { editingProfileId = p.id; renderOverlayBody() })
    chips.append(chip)
  }
  // 命中状态（本月窗口）：档案是否实际承接了计费
  ...
  if (editing) wrap.append(profileForm(editing))
    ...
    const hitHint = profilesData.length > 0
      ? `本月被命中：${profilesData.filter((p) => hitProfiles.has(p.id)).map((p) => p.label).join('、') || '（无——计费已全部由模型规则承接）'}`
      : '尚无档案'
    wrap.append(el('div', { class: 'dab-hint' },
      '计费优先级：模型计费规则（官方库/手动/智能）→ 账号档案兜底 → 未计费。\n' +
...
/** 预算·汇率视图 */
function budgetView(): HTMLElement {
  const wrap = el('div', {})
  const dailyIn = input(statusData?.budgets?.daily?.limit !== null && statusData?.budgets?.daily?.limit !== undefined ? String(statusData.budgets.daily.limit) : '', '¥/日')
  const monthlyIn = input(statusData?.budgets?.monthly?.limit !== null && statusData?.budgets?.monthly?.limit !== undefined ? String(statusData.budgets.monthly.limit) : '', '¥/月')
  ...
  saveSettings.addEventListener('click', () => {
    void (async () => {
      ...
      await apiJson('/settings', { method: 'PUT', body: JSON.stringify({ budgets: { daily: num(dailyIn.value), monthly: num(monthlyIn.value) }, refreshMs: num(refreshIn.value) ?? 1000, fx: { usdToCny: num(fxIn.value) ?? 7.2 } }) })
      toast('已保存'); window.dispatchEvent(new CustomEvent('dab-retune')); await refresh()
    })().catch((e) => toast(String(e).slice(0, 160), true))
  })
  wrap.append(
    el('div', { class: 'dab-sec' }, '预算（人民币口径，credit 模型不参与预算）· 刷新 · 汇率'),
    el('div', { class: 'dab-form' }, field('日预算（¥）', dailyIn), field('月预算（¥）', monthlyIn), field('徽标/面板刷新', refreshIn), field('美元→人民币汇率', fxIn)),
    el('div', { class: 'dab-btnrow' }, saveSettings),
  )
  return wrap
}

/** 导入导出视图 */
function ioView(): HTMLElement {
  const wrap = el('div', {})
  ...
  exportBtn.addEventListener('click', () => {
    void (async () => {
      ...
      a.href = URL.createObjectURL(blob); a.download = `dsh-agent-billing-${new Date().toISOString().slice(0, 10)}.json`; a.click()
  ...
  fileIn.addEventListener('change', () => {
    void (async () => {
      ...
      await apiJson('/import', { method: 'POST', body: JSON.stringify({ profiles: data.profiles, replaceSamples: data.samples ? { samples: data.samples } : undefined }) })
      toast('导入完成'); await reloadProfiles(); await refresh()
    })().catch((e) => toast(String(e).slice(0, 160), true))
  })
...
${(statusData?.degraded ?? []).join('\n')}`))
  wrap.append(el('div', { class: 'dab-hint', style: 'margin-top:14px' }, `账本与配置存储在 DSH_HOME/dsh-agent-billing/（ledger.sqlite / profiles.json / settings.json / model-rules.json）。卸载插件不删数据，重装即恢复。dsh-agent-billing v${statusData?.version ?? '?'}`))
  return wrap
}

function validateClient(rule: any): void {
  if (!rule.key || typeof rule.key !== 'string') throw new Error('规则 key 缺失')
  if (!['CNY', 'USD', 'credit'].includes(rule.currency)) throw new Error('币种非法：' + rule.currency)
  if (rule.currency === 'credit' && !(Number(rule.creditFactor) > 0)) throw new Error('credit 币种必须填写 credit 系数')
}


async function loadRules(): Promise<void> {
  try { rulesData = (await apiJson('/model-rules')).rules ?? [] } catch { rulesData = [] }
}
...
function renderOverlayBody(): void {
  if (overlayRendering) return // 重入防护：同步递归渲染会把页面拖死
  overlayRendering = true
  try {
    for (const overlay of [...overlayEls]) {
      if (!overlay.isConnected) { overlayEls.delete(overlay); continue }
      const body = overlay.querySelector('.dab-body')
      if (!body) continue
      const win = statusData?.windows?.[activeWindow]
      body.replaceChildren()
      if (!statusData) { body.append(el('div', { class: 'dab-hint' }, '连接 Host 中…')); continue }
  ...
  } finally { overlayRendering = false }
}

function buildOverlay(): HTMLElement {
  ensureStyle()
  const root = el('div', { class: 'dab-overlay' })
  // 显隐与全屏定位走 inline style：类样式被外部清理时也绝不出现"幽灵面板"
  ...
  const tabs: Array<['overview' | 'detail' | 'sessions' | 'settings', string]> = [['overview', '概览'], ['detail', '明细'], ['sessions', '会话'], ['settings', '设置']]
  const wins: Array<['today' | 'month' | 'total', string]> = [['today', '今日'], ['month', '本月'], ['total', '累计']]
  const rebuild = (): void => {
    tabbar.replaceChildren(...tabs.map(([key, label]) => {
      const b = el('button', { class: `dab-tab${activeTab === key ? ' active' : ''}` }, label)
      b.addEventListener('click', () => { activeTab = key; if (key === 'settings') void reloadProfiles().then(renderOverlayBody); rebuild(); renderOverlayBody() })
      return b
    }))
    winSwitch.replaceChildren(...wins.map(([key, label]) => {
      ...
      b.addEventListener('click', () => { activeWindow = key; rebuild(); renderOverlayBody() })
      return b
    }))
  }
  rebuild()
  ...
  const onToggle = (): void => {
    setOpen(!overlayOpen)
    if (overlayOpen) { if (activeTab === 'settings') void reloadProfiles(); void refresh() }
  }
  const onKey = (e: KeyboardEvent): void => { if (e.key === 'Escape' && overlayOpen) setOpen(false) }
  window.addEventListener(TOGGLE_EVENT, onToggle)
  window.addEventListener('keydown', onKey)
  ;(root as any).__dabDispose = (): void => {
    window.removeEventListener(TOGGLE_EVENT, onToggle)
    window.removeEventListener('keydown', onKey)
  }
  const panel = el('div', { class: 'dab-panel' },
    el('div', { class: 'dab-head' },
      el('span', { class: 'dab-title' }, '◈ AGENT BILLING'),
      tabbar, winSwitch,
      el('button', { class: 'dab-close', onclick: () => setOpen(false) }, '✕'),
    ),
    body,
  )
  root.append(panel)
  overlayEls.add(root)
  ;(root as any).__dabMount = (): void => renderOverlayBody()
  return root
}
...
function toast(message: string, isError = false): void {
  ...
  const node = el('div', { class: `dab-toast${isError ? ' err' : ''}`, id: 'dab-toast' }, message)
...
export function apply(ctx: ClientContext): void {
  ensureStyle()
  // B2 实时徽标：会话顶栏 headerUtilities，紧贴「📱 Android 面板」左侧
  // （android-pane 注册 order=-1，slot 按 order 升序 → -2 排其前 = 紧靠左边）
  ctx.effect(() => ctx.slots.inject('conversation.session.header.utilities', () =>
    ctx.slots.register({
      name: 'conversation.session.header.utilities',
      id: `${NS}:badge`,
      order: -2,
    }, function BillingBadge(): any {
      return React.createElement(VanillaSlot, { build: buildBadge })
    }),
  ), `${NS}:badge`)
  // B1 详细面板：全屏 overlay
  ctx.effect(() => ctx.slots.inject('shell.overlay', () =>
    ctx.slots.register({
      ...
      id: `${NS}:overlay`,
      order: 100,
    }, function BillingOverlay(): any {
      return React.createElement(VanillaSlot, { build: buildOverlay })
    }),
  ), `${NS}:overlay`)
  startPolling(ctx)
  void reloadProfiles()
